...
function renderTable(data) {
  ...
  const headerCells = columns.map(c => `<th>${c.label}</th>`).join('') +
    ((data.summary || []).length ? '<th>Итого</th>' : '');
  ...
  const rows = data.steps.map((step, i) => {
    ...
    return `<tr class="${i % 2 === 0 ? 'even' : 'odd'}">${cells}${summaryCell}</tr>`;
  }).join('');
...
function buildHtml(pages) {
  const sections = pages.map(renderTable).join('');
  return `<!DOCTYPE html>
...
async function renderPng(pages) {
