...
const { repairPageSummary, buildGroupByStep, isInsideSummarySpan } = require('./summary-groups');

function getColumns(danceType) {
  const countLabel = danceType === 'viennese-waltz' ? 'Счёт' : 'Ритм';
  const base = [
    { key: 'step', label: 'Шаг' },
  ...
  if (danceType === 'tango') return [...base, { key: 'beats', label: 'Счёт' }];
  if (danceType === 'slow-fox') return [...base, { key: 'beats', label: 'Счёт' }, { key: 'sway', label: 'Свей' }];
  return [...base, { key: 'sway', label: 'Свей' }];
}

function renderTable(data, danceType = 'slow-waltz') {
  const { repairNotes } = require('./notes');
  repairNotes(data);
  if (danceType === 'tango' || danceType === 'slow-fox') repairPageSummary(data, danceType);
  const columns = getColumns(danceType);
  ...
  const rows = data.steps.map((step, i) => {
    ...
    const cells = columns.map(col => {
      ...
      if (col.key === 'count') {
        if (danceType === 'viennese-waltz') return `<td>${val}</td>`;
    ...
    if (sum && !isInsideSummarySpan(stepNum, data.summary)) {
  ...
  const { getLeadBlockTitle, getNotesSectionTitle } = require('./notes');
  const notes = (data.notes || []).map(n => `<li>${n}</li>`).join('');
  const hasNotesList = (data.notes || []).length > 0;
  const leadTitle = getLeadBlockTitle(data.notesHeader, hasNotesList, data.title);
  const leadBlock = data.leadNotes ? `<div class="notes"><h4>${leadTitle}</h4><p>${data.leadNotes}</p></div>` : '';
  const notesTitle = getNotesSectionTitle(data.notesHeader, !!data.leadNotes, data.title);
  ...
  return `
    <div class="section">
      ...
      ${leadBlock}
      ${notes ? `<div class="notes"><h4>${notesTitle}</h4><ol>${notes}</ol></div>` : ''}
    </div>
  `;
}
